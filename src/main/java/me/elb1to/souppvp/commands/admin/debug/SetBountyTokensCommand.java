...
public class SetBountyTokensCommand extends BaseCommand {
    @Override
    @Command(name = "settokens", aliases = "settokens", permission = "soup-pvp.admin")
    public void onCommand(CommandArgs command) {
        Player player = command.getPlayer();
        String[] args = command.getArgs();

        if (args.length < 2) {
            player.sendMessage(ColorHelper.translate("&cUsage: /settokens <player> <amount>"));
            return;
        }
        ...
        targetUser.setBounty(Integer.parseInt(args[1]));
        player.sendMessage(ColorHelper.translate("&aSuccessfully modified " + target.getName() + "'s tokens amount to " + args[1] + "."));
    }
}
